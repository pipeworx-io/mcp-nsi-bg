...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/nsi_bg_list_datasets \
  -H 'Content-Type: application/json' \
  -d '{"filter":"population"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/nsi_bg_list_datasets`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
